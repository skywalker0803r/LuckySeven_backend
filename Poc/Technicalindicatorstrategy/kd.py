...
def detect_kd_signal(df: pd.DataFrame, k_period=14, d_period=3) -> pd.DataFrame:
    df = df.copy()
    ...
    df["signal"] = 0
    cross_up = (df["%K"] > df["%D"]) & (df["%K"].shift(1) <= df["%D"].shift(1)) & (df["%K"] < 30)
    cross_down = (df["%K"] < df["%D"]) & (df["%K"].shift(1) >= df["%D"].shift(1)) & (df["%K"] > 70)
