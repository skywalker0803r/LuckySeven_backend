...
def detect_willr_signal(df: pd.DataFrame, period: int = 14) -> pd.DataFrame:
    df = df.copy()
    highest_high = df["high"].rolling(period).max()
    lowest_low = df["low"].rolling(period).min()
    df["%R"] = -100 * (highest_high - df["close"]) / (highest_high - lowest_low + 1e-9)
    df["signal"] = 0
    df.loc[(df["%R"] > -80) & (df["%R"].shift(1) <= -80), "signal"] = 1
    df.loc[(df["%R"] < -20) & (df["%R"].shift(1) >= -20), "signal"] = -1
    return df
