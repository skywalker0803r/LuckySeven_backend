...
def detect_adx_signal(df: pd.DataFrame, period: int = 14) -> pd.DataFrame:
    df = df.copy()
    high = df["high"]
    low = df["low"]
    close = df["close"]
    ...
    df["signal"] = 0
    buy = (adx > 25) & (plus_di > minus_di) & (plus_di.shift(1) <= minus_di.shift(1))
    sell = (adx > 25) & (plus_di < minus_di) & (plus_di.shift(1) >= minus_di.shift(1))
    df.loc[buy, "signal"] = 1
